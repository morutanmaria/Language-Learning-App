...
export const Card = ({id, imageSrc, audioSrc, text, shortcut, selected, onClick, disabled, status, type}: Props) => {
    ...
    return (
        <div onClick={handleClick} className={cn("h-full border-2 rounded-xl border-b-4 hover:bg-black/5 p-5 lg:p-4 cursor-pointer active:border-b-2",
            selected && "border-sky-300 bg-sky-100 hover:bg:sky-100",
            selected && status === "correct" && "border-green-300 bg-green-100 hover:bg:green-100",
            selected && status === "wrong" && "border-rose-400 bg-rose-200 hover:bg:rose-200",
            ...
            {audio}
            {imageSrc && (
                <div className="relative aspect-square mb-4 max-h-32 lg:max-h-37.5 w-full">
